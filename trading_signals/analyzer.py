"""
LLM analysis for TRADING SIGNALS.
...
def analyze_content(
    ...
    Args:
        ...
        source_hint: Content type hint (url, article, tweet, etc.)
        provider: 'ollama' or 'openai'
        tier_override: Force a specific tier (light/default/deep), or None for auto.

    Returns:
        Parsed JSON dict with financial analysis fields.
...
def generate_strategies(
    ...
    provider: str = "ollama",
    tier_override: str | None = "default",
) -> dict:
    """Generate trading strategies from a financial analysis.

    Args:
        analysis: The financial analysis dict from analyze_content()
        portfolio_context: Optional string describing current holdings, risk tolerance, etc.
        provider: 'ollama' or 'openai'
        tier_override: Model tier override

    Returns:
        Dict with strategies list, or error dict.
    """
    from trading_signals.config import STRATEGY_PROMPT

    content = json.dumps(analysis, indent=2)
    if portfolio_context:
        content += f"\n\n--- Portfolio Context ---\n{portfolio_context}"

    user_prompt = "Generate trading strategies for this analysis:\n\n---\n" + content[:8000] + "\n---"

    model = pick_model("article", provider, tier_override)
    ...
    try:
        ...
        return result if isinstance(result, dict) else {"strategies": [], "error": "Unexpected format"}
    except Exception as e:
